...
const DINOS: Dino[] = [
  {
    id: 'trex', ko: '티라노사우루스', en: 'Tyrannosaurus Rex', wiki: 'Tyrannosaurus_rex',
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-4 py-3 shadow-sm">
        <input type="text" placeholder="🔍 공룡 이름 검색..." value={search}
      ...
      <div className="grid grid-cols-2 gap-3 p-4 max-w-2xl mx-auto pb-12">
        ...
        {filtered.length === 0 && (
          <div className="col-span-2 text-center py-16 text-gray-400">
            <p className="text-4xl mb-2">🦕</p><p className="text-sm">검색 결과가 없어요</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center"
          ...
          <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl max-h-[92vh] overflow-y-auto relative"
            ...
            <button onClick={() => setSelected(null)}
              className="absolute top-3 right-3 bg-gray-100 rounded-full w-8 h-8 flex items-center justify-center text-gray-500 font-bold text-sm">✕</button>
            <div className="p-6 pt-4">
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {selected.features.map(f => (
                  <span key={f} className="text-sm bg-emerald-50 text-emerald-700 border border-emerald-200 px-3 py-1 rounded-full font-medium">✦ {f}</span>
                ))}
              </div>
